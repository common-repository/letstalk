...
class Letstalk_Sanitize {
	...
	public function clean() {
		 ...
		 */
		do_action( 'topishare_pre_sanitize', $sanitized );
		...
		switch ( $this->type ) {
			...
			case 'textarea'			: $sanitized = $this->data; /*@@esc_textarea( $this->data );*/ break;
		 ...
		 */
		do_action( 'topishare_post_sanitize', $sanitized );
	...
	public function set_type( $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'letstalk' ) );

		}

		if ( is_wp_error( $check ) ) {

			wp_die( $check->get_error_message(), __( 'Forgot data type', 'letstalk' ) );
